...
parser.add_argument("--ckpt", type=str, default="checkpoints/mini_rt2_best.pt")
...
def main():
    ...
    tokenizer = ActionTokenizer(num_bins=num_bins)
    ...
    print("Render:", render)
    print("GL:", args.gl)
    print("Press q in camera window to stop current rollout.")
    ...
    for ep in range(args.episodes):
        ...
        for step in range(args.horizon):
            ...
            with torch.no_grad():
                pred_tokens = model.predict_tokens(image, text_ids)[0].detach().cpu().numpy()

            action = tokenizer.decode(pred_tokens)

            # Safety clamp
            action = np.clip(action * args.action_scale, -1.0, 1.0).astype(np.float32)
            ...
            title = (
                f"Mini RT-2 rollout ep={ep} step={step} "
                f"dz={lift_delta:.3f} tokens={pred_tokens.tolist()}"
            )

            stop = show_camera_panel(
                obs=obs,
                camera_keys=camera_keys,
                window_name="Mini RT-2 Rollout: agentview + frontview",
                title_text=title,
                delay=1,
            )
        ...
        video_path = f"videos/mini_rt2_rollout_ep_{ep:04d}.mp4"
